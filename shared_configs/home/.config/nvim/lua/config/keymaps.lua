...
vim.keymap.set({ 'n', 'v' }, '<localleader>f', ':HopWordCurrentLine<cr>', { desc = 'Hop Current Line' })
vim.keymap.set({ 'n', 'v' }, '<localleader>a', ':HopWord<cr>', { desc = 'Hop Word' })
vim.keymap.set({ 'n', 'v' }, '<localleader>n', ':HopNodes<cr>', { desc = 'Hop Nodes' })
vim.keymap.set({ 'n', 'v' }, '<localleader>l', ':HopLine<cr>', { desc = 'Hop Line' })
vim.keymap.set({ 'n', 'v' }, '<localleader>L', ':HopLineStart<cr>', { desc = 'Hop Start' })
vim.keymap.set({ 'n', 'v' }, '<localleader>y', ':HopYankChar1<cr>', { desc = 'Hop Yank' })
...
-- Diagnostic
vim.keymap.set('n', '<localleader>d', vim.diagnostic.goto_next, { desc = 'Diagnostic next' })
vim.keymap.set('n', '<localleader>D', vim.diagnostic.goto_prev, { desc = 'Diagnostic previous' })
...
vim.keymap.set('n', '<localleader><localleader>y', "<cmd>lua setopfn('v:lua.append_yank')<cr>g@", { desc = 'Yank append motion' })
vim.keymap.set('n', '<localleader><localleader>Y', "<cmd>lua setopfn('v:lua.append_yank')<cr>g@$", { desc = 'Yank append end' })
vim.keymap.set('n', '<localleader><localleader>yy', "<cmd>lua setopfn('v:lua.append_yank')<cr>g@V$", { desc = 'Yank append line' })

-- Telescope
-- vim.keymap.set("n", "<leader>sB", "<cmd>Telescope grep_string grep_open_files=true search=<cr>", { desc = "Grep bufs" })
vim.keymap.set('n', '<localleader><localleader>b', '<cmd>Telescope grep_string grep_open_files=true search=<cr>', { desc = 'Grep bufs' })

-- LF
local Util = require 'lazyvim.util'
local lfTermRoot = function() Util.terminal({ 'lf' }, { cwd = Util.root(), esc_esc = false, ctrl_hjkl = false }) end
local lfTermCurrent = function() Util.terminal({ 'lf' }, { esc_esc = false, ctrl_hjkl = false }) end

vim.keymap.set('n', '<localleader><localleader>l', lfTermRoot, { desc = 'LF (root dir)' })
vim.keymap.set('n', '<localleader><localleader>L', lfTermCurrent, { desc = 'LF (cwd)' })
